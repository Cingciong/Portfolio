...
<template>
    ...
    <section class="  bg-secondary pt-6 p-5 w-full bg-opacity-50 mx-auto rounded-3xl mt-7 mb-24   max-w-[1200px] relative grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5">
        ...
        <img v-for="card in cards" :key="card.id" :src="card.url" alt="Photo" class="w-full aspect-square object-center object-cover rounded-2xl tilt-card cursor-pointer" @click="openModal(card.url)">
    </section>
    ...
    <div v-if="selectedImage" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[100]" @click.self="closeModal">
        ...
        <button @click="closeModal" class="absolute top-5 right-12 text-white text-[45px]">&times;</button>
    </div>
</template>
