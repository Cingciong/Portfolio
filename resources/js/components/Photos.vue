...
const usePhotoFolder = ref(true);

const loadPhotos = async () => {
    loading.value = true;
    await Album.loadPhotos(usePhotoFolder.value ? 'photo' : 'work');
...
<template>
    ...
    <div class="flex justify-center mb-4">
        <label class="flex items-center cursor-pointer">
            ...
            <input type="checkbox" v-model="usePhotoFolder" class="hidden">
            <div class="relative">
                <div class="block bg-gray-600 w-14 h-8 rounded-full"></div>
                <div class="dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition" :class="{ 'translate-x-full': !usePhotoFolder.value }"></div>
            </div>
    ...
    <div class="bg-secondary pt-6 p-5 w-[1250px] bg-opacity-50 mx-auto rounded-3xl mt-7 mb-24">
        <section class="container mx-auto w-[1200px] min-h-screen relative">
            <div v-if="loading" role="status" class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-center justify-center p-5 z-50 bg-gray-500 bg-opacity-50">
