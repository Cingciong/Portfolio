...
onMounted(() => {
    ...
    // Wait for the router-view to be fully loaded
    router.isReady().then(() => {
...
<template>
    <div :class="{'hidden': !isLoading}"  class="fixed inset-0 flex items-center justify-center bg-primary z-50 min-h-screen">
    ...
    <main id="background" class="overflow-hidden relative bg-primary font-main h-fit flex flex-col gap-20 w-full text-text animate-duration-600">
        ...
        <div id="follow-mouse" class="w-[400px] h-[400px] bg-gradient-to-r from-teal-400/60 to-yellow-200/60 rounded-full pointer-events-none blur-3xl fixed translate-y-1/2 translate-x-1/2"></div>
